        ...
        $treeBuilder->getRootNode()
            ->children()
                ...
                ->scalarNode(ContentInterface::AUTHOR)
                    ...
                    ->defaultValue('')
                ->end()
                ->scalarNode(ContentInterface::CREATE_DATE)
                    ->info('Page date in Unix timestamp format.')
                    ->defaultValue(time())
                ->end()
                ->integerNode(ContentInterface::MENU_ITEM_NUMBER)
                    ->min(-1)
                    ->defaultValue(-1)
                ->end()
                ->booleanNode(ContentInterface::SKIP_FROM_LISTING)
                    ->info('Page will not be present in listings like the newest etc.')
                    ->defaultValue(false)
                ->end()
